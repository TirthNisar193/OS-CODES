...
struct node{
    int at, bt, ct, wt, tat, start, id, com, r_bt, priority;
}pq[10];
...
void main(){
    int n,current=0,atat=0,awt=0,completed=0,high_priority_job=0;
    ...
    while(completed<n){
        ...
        if(pq[high_priority_job].r_bt == pq[high_priority_job].bt){
            pq[high_priority_job].start = current;
        }
        pq[high_priority_job].r_bt--;
        current++;
        if(pq[high_priority_job].r_bt <= 0){
            pq[high_priority_job].com = 1;
            pq[high_priority_job].ct = current;
            completed++;
        }
    }
    ...
    printf("\n");
    for(int i=0; i<n; i++){
        printf("PID : %d, AT : %d, BT : %d, CT : %d, TAT : %d, WT : %d, Priority : %d\n", pq[i].id, pq[i].at, pq[i].bt, pq[i].ct, pq[i].tat, pq[i].wt, pq[i].priority);
    }
    printf("\n");
    for(int i=0; i<n; i++){
